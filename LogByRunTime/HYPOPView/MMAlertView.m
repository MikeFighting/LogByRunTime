                 ...
                 inputHandler:(MMPopupInputHandler)inputHandler
{
    ...
    if ( self )
    {
        ...
        if ( detail.length > 0 )
        {
            ...
            //self.detailLabel.textAlignment = config.detailTextAlignment; // NSTextAlignmentCenter;
            ...
            [self setLabelLineSpace:self.detailLabel];
        ...
        [lastButton mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if ( items.count <= 2 )
            {
                ...
                if (items.count == 1) {
                    ...
                    lastButton.layer.borderColor = config.itemHighlightColor.CGColor;
                }
            }
            else
            {
...
-(void)setLabelLineSpace:(UILabel *)lblResult
{
    CGFloat lineSpace = 5;
    ...
    paragraphStyle.lineSpacing = lineSpace; // 调整行间距
    NSDictionary *attrs = @{
                            NSFontAttributeName : lblResult.font,
    ...
    CGFloat rowHeight = [lblResult.text boundingRectWithSize:CGSizeMake(screenWidth-marginLeft*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size.height;
    // 如果超出一行，则offset=0;
    offset = rowHeight > oneHeight ? 0 : offset;
    lblResult.attributedText = [self getAttributedStringWithString:lblResult.text lineSpace:lineSpace baselineOffset:offset];
}
